#ifndef MX64_DRIVER_HPP
#define MX64_DRIVER_HPP
...
{
    public:
        MX64Driver(std::string device_name)
        {
        ...
        void activateWithPositionMode(uint8_t dxl_id)
        {
        ...
        void activateWithVelocityMode(uint8_t dxl_id)
        {
            std::cout << "Activate motor" << std::endl;
        ...
        void deactivate(uint8_t dxl_id)
        {
        ...
        void setTargetPositionRadian(uint8_t dxl_id, double command) 
        {
            int dxl_cmd = command * RAD_TO_DXL_POSITION + 2048;
            packetHandler_->write4ByteTxRx(portHandler_, dxl_id, ADDR_GOAL_POSITION, dxl_cmd);
        }
    
        void setTargetVelocityRadianPerSec(uint8_t dxl_id, double command)
        {
        ...
        double getPositionRadian(uint8_t dxl_id) 
        {
        ...
        double getVelocityRadianPerSec(uint8_t dxl_id)
        {
